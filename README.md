...
Windows 打包脚本生成托盘版程序：`build-windows.ps1`。显卡后端按平台选择：NVIDIA 使用 `nvidia-smi`，Windows 无 NVIDIA 时读取 GPU Engine，Linux 读取 DRM/sysfs，Apple Silicon 读取 IOAccelerator。托盘入口在 `cmd/monitor-tray`。
